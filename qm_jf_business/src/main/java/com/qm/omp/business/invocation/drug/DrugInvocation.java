...
//		endTime = DateTimeUtil.get_yyyyMMddHHmmss(endTime);
    ...
    public InvocationResult queryStock(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = drugService.queryStock(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public InvocationResult getListByUser(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = drugService.getListByUser(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
     ...
     * @throws
     */
    public InvocationResult getListByUser_ExceptSelected(InvocationContext context)
    {
    	// 返回结果对象
    	InvocationResult result = InvocationResult.newInstance();
    	int page = RequestUtil.getIntParamterAsDef(context.getRequest(), "page", 1);
    	int rows = RequestUtil.getIntParamterAsDef(context.getRequest(), "rows", 50);
    	int tax = RequestUtil.getIntParamterAsDef(context.getRequest(), "tax", -1);
    	String fName = RequestUtil.getStrParamterAsDef(context.getRequest(), "fName", "");
    	String fCityName = RequestUtil.getStrParamterAsDef(context.getRequest(), "fCityName", "");
    	String ids = RequestUtil.getStrParamterAsDef(context.getRequest(), "ids", "");//已选择的药品id
    	String nums = RequestUtil.getStrParamterAsDef(context.getRequest(), "nums", "");//已选择的药品销售情况
    	String fCompanyIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "fCompanyIds", "");//客户所在公司
    	String fDrugPrinterIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "fDrugPrinterIds", "");//客户经营分类
    	Map<String, Object> params = new HashMap<String, Object>();
    	params.put("fName", fName);
    	params.put("fCityName", fCityName);
    	/**
    	...
    	try
    	{
    		Map<String, Object> map = drugService.getListByUser_ExceptSelected(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    		logger.error("DrugInvocation getListByUser_ExceptSelected happen execption.", ex);
    		result.setRetCode(ISystemConstants.SYS_FAILED);
    	}
    	return result;
    }
    ...
    public InvocationResult save(InvocationContext context){
    	...
    	String fBatchNumber = RequestUtil.getStrParamterAsDef(context.getRequest(), "fBatchNumber", "");
    	String fExpiryDate = RequestUtil.getStrParamterAsDef(context.getRequest(), "fExpiryDate", "");
    	int fNumber = RequestUtil.getIntParamter(context.getRequest(), "fNumber");
    	String fPrice = RequestUtil.getStrParamterAsDef(context.getRequest(), "fPrice", "");
    	String fBuyingPrice = RequestUtil.getStrParamterAsDef(context.getRequest(), "fBuyingPrice", "");
    	String fGongyePrice = RequestUtil.getStrParamterAsDef(context.getRequest(), "fGongyePrice", "");
    	int fWareHouseId = RequestUtil.getIntParamter(context.getRequest(), "fWareHouseId");
    	int fDrugOnlyId = RequestUtil.getIntParamter(context.getRequest(), "fDrugOnlyId");
    	int fCompanyId = RequestUtil.getIntParamter(context.getRequest(), "fCompanyId");
    ...
    public InvocationResult userDrug(InvocationContext context)
    {
        ...
        String drugIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "drugIds", "");
    ...
    public InvocationResult userDrugOnly_setPrice(InvocationContext context)
    {
    	...
    	int drugOnlyId = RequestUtil.getIntParamter(context.getRequest(), "drugOnlyId");//药品id
    	Double fSupplyPrice = Double.parseDouble(RequestUtil.getStrParamterAsDef(context.getRequest(), "fSupplyPrice", "0"));//供货价
    	Double fRetailPrice = Double.parseDouble(RequestUtil.getStrParamterAsDef(context.getRequest(), "fRetailPrice", "0"));//零售价
    	
    	try
    	{
    		this.drugService.userDrugOnly_setPrice(userId, drugOnlyId, fSupplyPrice, fRetailPrice);//先删除再添加
    	}
    	catch (Exception ex)
    	{
    		logger.error("DrugInvocation userDrugOnly_setPrice happen execption.", ex);
    		result.setRetCode(ISystemConstants.SYS_FAILED);
    	}
    	return result;
    }
    ...
    public InvocationResult userDrugOnly_showPrice(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = this.drugService.userDrugOnly_showPrice(userId, drugOnlyId);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    		logger.error("DrugInvocation userDrugOnly_showPrice happen execption.", ex);
    		result.setRetCode(ISystemConstants.SYS_FAILED);
    	}
    	return result;
    }
    ...
    public InvocationResult checkExpireDate(InvocationContext context)
    {
    	...
    	String ids = RequestUtil.getStrParamterAsDef(context.getRequest(), "ids", "");//已选择的药品id
    	String nums = RequestUtil.getStrParamterAsDef(context.getRequest(), "nums", "");//已选择的药品销售情况
    	String fCompanyIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "fCompanyIds", null);//客户所在公司
    	String fDrugPrinterIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "fDrugPrinterIds", null);//客户经营分类
    	Map<String, Object> params = new HashMap<String, Object>();
			...
			if(map != null){
				...
				for(Entry<Integer, Integer> en : map.entrySet()){
					while(drugL.hasNext()){//如果和已选择的药品id相同并且库存和销量一致则删除该药（隐藏药品）,这里收集需要排除掉的药品id
						bean = drugL.next();
						if(bean.getfId().intValue() == en.getKey().intValue() && bean.getfNumber().intValue() == en.getValue().intValue()){
    		...
    		if(drugId == list.get(0).getfId().intValue()){//如果选择的效期是未选择的最近效期，也就是排序后的数据的第一个则可以添加，不然则报不可以
    			...
    			if(map != null){
    				for(Entry<Integer, Integer> en : map.entrySet()){
    					...
    					if(drugId == drugIdSel && fSalesNumber == list.get(0).getfNumber()){//该药品已被选择,并且销售数量和库存一致（表明最近效期的药品已经卖完了，可以点其他效期的）
