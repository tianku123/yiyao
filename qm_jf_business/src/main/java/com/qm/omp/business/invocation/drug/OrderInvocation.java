...
public class OrderInvocation implements BaseInvocation{
    ...
    public InvocationResult getList_ywy(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = orderService.getList_ywy(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public InvocationResult getList_zg(InvocationContext context)
    {
    	...
    	params.put("beginTime", com.qm.omp.business.util.DateTimeUtil.get_yyyyMMddHHmmss(beginTime));
    	params.put("endTime", com.qm.omp.business.util.DateTimeUtil.get_yyyyMMddHHmmss(endTime));
    	try
    	{
    		Map<String, Object> map = this.orderService.getList_zg(params, page, rows, userInfo.getfUserRoleRel());
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public InvocationResult getTc_Total(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = this.orderService.getTc_Total(params, userInfo.getfUserRoleRel());
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public InvocationResult getList_policy(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = orderService.getList_policy(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public InvocationResult getList_cw(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = orderService.getList_cw(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public InvocationResult getList_fh(InvocationContext context)
    {
    	...
    	try
    	{
    		Map<String, Object> map = orderService.getList_fh(page, rows, params);
    		result.setRetObj(map);
    	}
    	catch (Exception ex)
    	{
    ...
    public synchronized InvocationResult save(InvocationContext context){
    	...
    	String drug = RequestUtil.getStrParamterAsDef(context.getRequest(), "drug", "");
    	String customer = RequestUtil.getStrParamterAsDef(context.getRequest(), "customer", "");
    	...
    	if("0".equals(fTax)){//不含税  0：不含税，1：含税(增值税)，2：含税(普通)
    		for(int i=0; i<dArr.size(); i++){
    			...
    			//单个药品返点= 数量*单价 * 0.07
//    			fFanDian = fPrice.doubleValue() * fSalesNumber.intValue() * 0.07;
    			fFanDian = 0d;
    			//主订单合计返点金额
    			...
    			//金额
    			fMoney_buyingPrice = fBuyingPrice.doubleValue() * fSalesNumber.intValue();
    			//合计金额
    		...
    		for(int i=0; i<dArr.size(); i++){
    			...
    			fDqTc = obj.getDouble("fDqTc");//大区提成
    			//过票费= 开票价 * 数量 * 0.03
    			fGuoJiFei = fKaiPiaoPrice.doubleValue() * fSalesNumber.intValue() * 0.03;
    			fGuoJiFeiTotal += fGuoJiFei;
    			//高开费 = （开票价-工业票价）* 数量 * 0.17
    			fGaoKaiFei = (fKaiPiaoPrice.doubleValue() - fPrice.doubleValue()) * fSalesNumber.intValue() * 0.17;
    			fGaoKaiFeiTotal += fGaoKaiFei;
    			//单个药品合计
    			fMoney = fPrice.doubleValue() * fSalesNumber.intValue() + fGuoJiFei + fGaoKaiFei;
    			//合计
    			fMoneyTotal += fMoney;
...
//    			fFanDian = fPrice.doubleValue() * fSalesNumber.intValue() * 0.07;
    			...
    			fFanDianTotal += fFanDian;
    			//单个药品合计金额
    			fMoney = fPrice.doubleValue() * fSalesNumber.intValue() - fFanDian;
    			//主订单合计金额
    			fMoneyTotal += fMoney;
    ...
    public synchronized InvocationResult editOrder(InvocationContext context){
    	...
    	// 返回结果对象
    	InvocationResult result = InvocationResult.newInstance();
    	String orderId = RequestUtil.getStrParamterAsDef(context.getRequest(), "fId", "");
    	String fTax = RequestUtil.getStrParamterAsDef(context.getRequest(), "fTax", "");
    	String fName = RequestUtil.getStrParamterAsDef(context.getRequest(), "fName", "");
    	String fAddress = RequestUtil.getStrParamterAsDef(context.getRequest(), "fAddress", "");
    	String fPhone = RequestUtil.getStrParamterAsDef(context.getRequest(), "fPhone", "");
...
//    			fFanDian = fPrice.doubleValue() * fSalesNumber.intValue() * 0.07;
     ...
     * @return
     */
    public synchronized InvocationResult editOrder_policy(InvocationContext context){
...
//    			fFanDian = fPrice.doubleValue() * fSalesNumber.intValue() * 0.07;
    ...
    public synchronized InvocationResult edit(InvocationContext context){
    	...
    	String fState = RequestUtil.getStrParamterAsDef(context.getRequest(), "fState", "");
    	String fExpressId = RequestUtil.getStrParamterAsDef(context.getRequest(), "fExpressId", "");
    	String fExpressName = RequestUtil.getStrParamterAsDef(context.getRequest(), "fExpressName", "");
    	String fShipperIntro = RequestUtil.getStrParamterAsDef(context.getRequest(), "fShipperIntro", "");
    	String fFinanceIntro = RequestUtil.getStrParamterAsDef(context.getRequest(), "fFinanceIntro", "");
    	String fSaleIntro = RequestUtil.getStrParamterAsDef(context.getRequest(), "fSaleIntro", "");
    	String fPaymentState = RequestUtil.getStrParamterAsDef(context.getRequest(), "fPaymentState", "");
    	Integer fPaymentSource = RequestUtil.getIntegerParamter(context.getRequest(), "fPaymentSource");
    	String fPolicyIntro2 = RequestUtil.getStrParamterAsDef(context.getRequest(), "fPolicyIntro2", "");
    	Order bean = new Order();
    	UserInfoBean userInfo = (UserInfoBean) SessionUtil.getObjectAttribute(context.getRequest(), RequestConstants.ADMIN_SESSION_KEY);
    	bean.setfId(fId);
    	bean.setfState(fState);
    	//状态：0，业务员未提交；1，提交财务审核，2，财务审核通过，3,已发货,4，退单
    	if("1".equals(fState)){//提交给财务审核
    		bean.setfSaleTime(DateTimeUtil.getTodayChar14());
    		bean.setfSaleUserId(userInfo.getId());
        	bean.setfSaleUserName(userInfo.getfUserCode());
        	bean.setfSaleIntro(fSaleIntro);
    	}else if("2".equals(fState)){//财务审核通过
    		...
    		//退单恢复订单内所有药品的库存
    		List<OrderDetail> list = this.orderDetailService.getListByOrderId(fId);
    	...
    	}else if("10".equals(fState)){//提交政策报单给管理员
    		bean.setfSaleTime(DateTimeUtil.getTodayChar14());
    		bean.setfSaleUserId(userInfo.getId());
        	bean.setfSaleUserName(userInfo.getfUserCode());
        	bean.setfSaleIntro(fSaleIntro);
    ...
    public synchronized InvocationResult editPrice(InvocationContext context){
    	...
    	String fTax = RequestUtil.getStrParamterAsDef(context.getRequest(), "fTax", "");
    	String drugIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "drugIds", "");//订单详细id,根据id对应药品
    	String prices = RequestUtil.getStrParamterAsDef(context.getRequest(), "prices", "");
    	String drugIds33 = RequestUtil.getStrParamterAsDef(context.getRequest(), "drugIds33", "");//订单详细id,根据id对应药品
    	String kaipiaoPrices = RequestUtil.getStrParamterAsDef(context.getRequest(), "kaipiaoPrices", "");
...
//    	bean.setfId(fOrderId);
    	 ...
    	 * 未修改的订单详细信息
    	 */
    	List<Map<String, Object>> detail = this.orderDetailService.getListMapByOrderId(fOrderId);
    	if(drugIds != null && !"".equals(drugIds)){
    		...
    		if("0".equals(fTax)){//不含税  0：不含税，1：含税(增值税)，2：含税(普通)
    			for(Map<String, Object> map : detail){
    				...
    				System.out.println(fPrice + "==" + fBuyingPrice + "==" + fSalesNumber);
    				//单个药品返点= 数量*单价 * 0.07
//    				fFanDian = fPrice.doubleValue() * fSalesNumber.intValue() * 0.07;
    				fFanDian = 0d;
    				//主订单合计返点金额
        		...
        		kaipiaoPrices = kaipiaoPrices.substring(0, kaipiaoPrices.length()-1);
        		...
        		for(int i=0; i<idList33.size(); i++){
        			drugKaiPiaoPriceMap.put(idList33.get(i), Double.parseDouble(arr33[i]));
        		}
    			for(Map<String, Object> map : detail){
    				fPrice = drugPriceMap.get(MapUtils.getString(map, "F_ID"));//单价
    				fBuyingPrice = MapUtils.getDouble(map, "F_BUYING_PRICE");//进货价
    				...
    				fSalesNumber = MapUtils.getInteger(map, "F_NUMBER");//销售数量
    				fKaiPiaoPrice = drugKaiPiaoPriceMap.get(MapUtils.getString(map, "F_ID"));//开票价
    				//过票费= 开票价 * 数量 * 0.03
    				fGuoJiFei = fKaiPiaoPrice.doubleValue() * fSalesNumber.intValue() * 0.03;
    				fGuoJiFeiTotal += fGuoJiFei;
    				//高开费 = （开票价-工业票价）* 数量 * 0.17
...
//    	bean.setfSaleUserName(userInfo.getfUserCode());
    	...
    	{
    		/*
    		 * 1：保留原有订单，这份订单是给业务员看的
    		 * 2：生成新订单，这份修改后的订单给财务和发货人员看
    		...
    		if(order.getfSourceId() == null){//没有原始订单id，首次被修改，生成新订单
    			...
    			List<OrderDetail> details = this.orderDetailService.getListByOrderId(fOrderId);
     ...
     * @return
     */
    public synchronized InvocationResult editPrice_policy(InvocationContext context){
    	
    	// 返回结果对象
    	InvocationResult result = InvocationResult.newInstance();
    	String fOrderId = RequestUtil.getStrParamterAsDef(context.getRequest(), "fOrderId", "");
    	String fTax = RequestUtil.getStrParamterAsDef(context.getRequest(), "fTax", "");
    	String nums = RequestUtil.getStrParamterAsDef(context.getRequest(), "nums", "");//订单详细id,
    	String drugIds = RequestUtil.getStrParamterAsDef(context.getRequest(), "drugIds", "");//订单详细id,
    	String prices = RequestUtil.getStrParamterAsDef(context.getRequest(), "prices", "");//单价
    	String fandians = RequestUtil.getStrParamterAsDef(context.getRequest(), "fandians", "");//返点
    	String moneys = RequestUtil.getStrParamterAsDef(context.getRequest(), "moneys", "");//金额

    	String kaipiaoPrices = RequestUtil.getStrParamterAsDef(context.getRequest(), "kaipiaoPrices", "");//开票价
    	String guopiaofeis = RequestUtil.getStrParamterAsDef(context.getRequest(), "guopiaofeis", "");//过票费
    	String gaokaifeis = RequestUtil.getStrParamterAsDef(context.getRequest(), "gaokaifeis", "");//高开费
...
//    				fandian = Double.parseDouble(fandianList.get(i));
    			...
    			kaipiaoPrices = kaipiaoPrices.substring(0, kaipiaoPrices.length()-1);//开票价
        		List<String> kaipiaoPriceList = Arrays.asList(kaipiaoPrices.split(","));
        		guopiaofeis = guopiaofeis.substring(0, guopiaofeis.length()-1);//过票费
        		List<String> guopiaofeiList = Arrays.asList(guopiaofeis.split(","));
        		gaokaifeis = gaokaifeis.substring(0, gaokaifeis.length()-1);//高开费
        		List<String> gaokaifeiList = Arrays.asList(gaokaifeis.split(","));
        		for(int i=0; i<drugIdList.size(); i++){
    				detail = new OrderDetail();
    				detail.setfOrderId(fOrderId);
    				detail.setfId(Integer.parseInt(drugIdList.get(i)));
    				detail.setfPrice(Double.parseDouble(priceList.get(i)));
    				detail.setfKaiPiaoPrice(Double.parseDouble(kaipiaoPriceList.get(i)));
    ...
    public InvocationResult salesSummary(InvocationContext context)
    {
    	...
    	String fType = RequestUtil.getStrParamterAsDef(context.getRequest(), "fType", "");
    	Map<String, Object> params = new HashMap<String, Object>();
    	params.put("fName", fName);
