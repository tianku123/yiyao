...
public class DrugServiceImpl{
	...
	public Map<String, Object> queryStock(int page, int rows, Map<String, Object> params) {
		page = (page-1)*rows;
		Map<String, Object> res = new HashMap<String, Object>();
		params.put("page", page);
		params.put("rows", rows);
	...
	public Map<String, Object> getListByUser_ExceptSelected(int page, int rows,
		...
		if(params.containsKey("selected")){
			...
			Map<Integer, Integer> sel = (Map<Integer, Integer>) params.get("selected");
			ListIterator<Drug> drugL = null;
			Drug bean = null;
			for(Entry<Integer, Integer> en : sel.entrySet()){
				drugL = data.listIterator();
				System.out.println("getListByUser_ExceptSelected==idList:"+ en.getKey().intValue());
			...
			System.out.println("getListByUser_ExceptSelected==idList:"+idList);
	...
	public void edit(Drug bean) {
		...
		int num = res.getfNumber().intValue() - bean.getfNumber().intValue();//原库存减去修改后的库存，50  100
	...
	public List<Drug> getDrugList(Integer userId) {
		...
		List<Map<String, Integer>> checkedList =  this.drugDao.getCheckedDrugList(userId);
	...
	public void userDrugOnly_setPrice(int userId, int drugOnlyId,
		...
		this.drugDao.userDrugOnly_setPrice(userId, drugOnlyId, fSupplyPrice, fRetailPrice);
	...
	public void minusNumber(Integer fDrugId, Integer fNumber) {
		this.drugDao.minusNumber(fDrugId, fNumber);
	}
