	...
	var merchantUserInfoComponent = {
		...
		init : function() {
			...
			var columns = [ 
				...
				{field : 'fBeginTime',title : '输入日期',width :100,align:'center',
					...
					for(var i=0;i<_arr.length;i++){
						...
						if(!merchantUserInfoComponent.removeDuplicate(fId)){//去重
							if(i==_arr.length-1){
								res += "{fId:'"+node['fId']
								+"',fName:'"+node['fName']
								+"',fCityName:'"+node['fCityName']
								+"',fDrugPrinterIds:'"+node['fDrugPrinterIds']
								+"',fCompanyIds:'"+node['fCompanyIds']
