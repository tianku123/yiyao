...
<body class="easyui-layout">
	<div data-options="region:'center',border:false" style="background-color:#FFF; padding:10px;">
		<table cellpadding="0" cellspacing="0" class="formTable" width="100%">
			<tr>
				<th width="15%"><label>登录名：</label>
				</th>
				<td width="35%"><input id="fUserCode" name="fUserCode" type="text" autofocus="autofocus" class="easyui-validatebox"
					data-options="required:true,validType:['queryMerUserByCode']" /></td>
				<th width="15%"><label>姓名：</label>
				</th>
				<td width="35%"><input id="fUserName" name="fUserName" type="text" autofocus="autofocus" class="easyui-validatebox" data-options="required:true" /></td>
			</tr>
			<tr>
				<th><label>所属角色：</label>
				</th>
				...
				<td><input id="fUserTel" name="fUserTel" type="text" autofocus="autofocus" class="easyui-validatebox"  /></td>
			</tr>
			<tr>
				<th><label>密码：</label>
				</th>
				<td><input id="fUserPwd" name="fUserPwd" type="password" autofocus="autofocus" class="easyui-validatebox" data-options="required:true" /></td>
				<th><label>确认密码：</label>
				</th>
				<td><input id="enterfUserPwd" type="password" autofocus="autofocus" class="easyui-validatebox" data-options="required:true" /></td>
			</tr>
			<tr>
				<th><label>所属区域：</label>
				</th>
...
<script>
	...
	var merchantUserInfoComponent = {
		init : function() {
			...
			$.fn.zTree.init($("#customer_tree"), {
				...
				data: {
					...
					onAsyncSuccess:function(event, treeId, treeNode, msg){
						/* var treeObj = $.fn.zTree.getZTreeObj("role_tree");
						var nodes = treeObj.getNodesByParam("fId", "${param.fId}", null);
						treeObj.selectNode(nodes[0]);
						$('#customer_combo').combo('clear').combo('setText',nodes[0]['fName']).combo('setValue',nodes[0]['fId']); */
					}
				}
		...
		comit : function() {
			...
			var fUserPwd = $("#fUserPwd").val();
			var enterfUserPwd = $("#enterfUserPwd").val();
			var result = $("#fUserCode").validatebox("isValid");
			if(result==false) {
	            return false;
			}
			...
			if ($.trim(fUserPwd)=='' || $.trim(enterfUserPwd)=='') {
				QM.dialog.showFailedDialog("密码不能为空！");
				return;
			}
			if ($.trim(fUserPwd) != $.trim(enterfUserPwd)) {
				QM.dialog.showFailedDialog("两次输入的密码不一致！");
				return;
			}
